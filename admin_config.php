...
<table cellspacing="0" cellpadding="5" border="0">
    ...
    <tr>
        <td width="50%" colspan="2">
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                    <td width="7%">
                        <input type="radio" name="auth_neon_locale" id="auth_neon_locale_en" class="auth_neon_locale_en"
                               value="en"<?php echo ($config->auth_neon_locale == 'en' || empty($config->auth_neon_locale)) ? ' checked="checked"' : ''; ?> />
                        <?php
                        if (isset($err['auth_neon_locale'])) {
                            echo $OUTPUT->error_text($err['auth_neon_locale']);
                        } ?>
                    </td>
                    <td width="43%">
                        <label
                            for="auth_neon_locale_en"><?php echo get_string('auth_neon_locale_en_key', 'auth_neon'); ?></label>
                    </td>
                    <td width="7%">
                        <input type="radio" name="auth_neon_locale" id="auth_neon_locale_ru" class="auth_neon_locale_ru"
                               value="ru"<?php echo ($config->auth_neon_locale == 'ru') ? ' checked="checked"' : ''; ?> />
                        <?php
                        if (isset($err['auth_neon_locale'])) {
                            echo $OUTPUT->error_text($err['auth_neon_locale']);
                        } ?>
                    ...
                    <td width="43%">
                        <label
                            for="auth_neon_locale_ru"><?php echo get_string('auth_neon_locale_ru_key', 'auth_neon'); ?></label>
                    </td>
                </tr>
            </table>
        </td>
        <td width="50%"><?php echo get_string('auth_neon_locale_desc', 'auth_neon'); ?></td>
    </tr>
    <!--tr>
    ...
    echo html_writer::checkbox(
        ...
        get_string('can_change_password', 'auth_neon'), array('id' => 'can_change_password')
    );
    ...
    <tr>
        <td width="50%" colspan="2"><?php
            echo html_writer::checkbox(
                'auth_neon_can_reset_password', 1,
                $config->auth_neon_can_reset_password,
                get_string('auth_neon_can_reset_password_key', 'auth_neon'), array('id' => 'auth_neon_can_reset_password')
            );
            if (isset($err['auth_neon_can_reset_password'])) {
                echo $OUTPUT->error_text($err['auth_neon_can_reset_password']);
            } ?>
        </td>
        <td width="50%"><?php echo get_string('auth_neon_can_reset_password_desc', 'auth_neon'); ?></td>
    </tr>
    <tr>
        <td width="50%" colspan="2"><?php
            echo html_writer::checkbox(
